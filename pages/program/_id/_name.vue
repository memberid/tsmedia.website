    ...
    <template v-if="selectProgram.assets.length">
      ...
      <div v-dragscroll.x class="program-wrapper">
        ...
        >
          ...
          <div class="program__button">
            <vs-button class="font-bold" blank :href="program.href">
              Watch on Youtube
            </vs-button>
          </div>
        ...
        >
          <img
            class="object-contain"
            :src="program.talentsBackdrop.url"
            alt=""
          />
        </div>
        <template v-if="program.assets.length">
          <div
            v-for="(item, idx) in program.assets"
            :key="idx"
            class="program__assets mx-12 lg:mx-20"
          ...
          >
            ...
            >
              <a
                :target="item.link !== null ? '_blank' : ''"
                :href="item.link !== null ? item.link : '#'"
              >
                <img class="object-contain" :src="item.image.url" alt="" />
              </a>
            </div>
          </div>
        </template>
...
export default {
  ...
  async asyncData({ $axios, $config, route }) {
    const program = await $axios
      .$get(`${$config.baseURL}/programs/${route.params.id}`)
      .then((res) => res)
  ...
  computed: {
    selectProgram() {
      const select = this.programs.filter((item) => {
